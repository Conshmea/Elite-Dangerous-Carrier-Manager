...
from config import PADLOCK, CD, CD_cancel, JUMPLOCK, ladder_systems, AVG_JUMP_CAL_WINDOW, ASSUME_DECCOM_AFTER, ladder_systems
...
class JournalReader:
    ...
    def __init__(self, journal_paths:list[str], dropout:bool=False, droplist:list[str]=None):
        ...
        if self.dropout == True:
            if self.droplist is None:
                ...
                self.droplist = [i for i in range(len(self.tracked_items) + 1) if random() < 0.5]
    ...
    def update_items_count(self):
        """
        Update the last items count to the current count, and remove any consumed items from the tracked lists.
        """
        for item_type in self.tracked_items:
            consumed = self._last_items_count_pending[item_type]
            if consumed:
                del getattr(self, f'_{item_type}')[:consumed]
        self._last_items_count = {item_type: 0 for item_type in self.tracked_items}
        self._last_items_count_pending = self._last_items_count.copy()
...
class CarrierModel:
    ...
    def process_squadrons(self, squadrons, first_read:bool=True):
        for squadron in squadrons:
            if squadron['FID'] is not None and (squadron['FID'] not in self.cmdr_squadrons.keys() or not first_read):
                self.cmdr_squadrons[squadron['FID']] = squadron['SquadronName']
        for carrierID in self.carriers.keys():
    ...
    def sorted_ids(self) -> list[int]:
        ...
        def system_to_ladder_index(system: str) -> int:
            wine_name = ladder_systems.get(system, None)
            if wine_name is not None:
                if wine_name.startswith('N') and wine_name[1:].isdigit():
                    return int(wine_name[1:])
                elif system == "Wine Cellar":
